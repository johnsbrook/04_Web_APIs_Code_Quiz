...
var imgStyle =
  ' style="width: 200px; height: auto; border-radius: 25px; box-shadow: 0px 0px 2px 2px lightgrey;';
...
function question2() {
  ...
  qp.append(row1);
  $("#row1").append(
    "<h1>In what city and country is the Taj Mahal Hotel?</h1>"
  );
  $("#row1").append(allAnswers.images1[1]);

  for (var i = 0; i < allAnswers.question2.length; i++) {
    $("#row1").append("<span>" + allAnswers.question2[i] + "</span>");
  }
...
function question4() {
  ...
  qp.append(row1);
  $("#row1").append(
    "<h1>Which city is known as the culinary capital of South America?</h1>"
  );
...
function enterScore() {
  ...
  qp.append(
    '<button id="reloadGame" class="StartQuiz col-10 col-md-3 mx-auto my-3 bg-primary text-white">' +
      "Restart Game" +
      "</button>"
  );
